...
Today we are going to load our ACS data that we have been working with all course long using `tidycensus` - a package that interfaces with the Census API.
...
Let's turn those NAs into zeroes using a mutate command.

We use an `ifelse` statement here - that's known as a "boolean" - remember that from our raster classes?
